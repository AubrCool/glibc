...
__do_cancel (void)
{
  ...
  self->cancelhandling |= CANCELSTATE_BITMASK;
  self->cancelhandling &= ~(CANCELTYPE_BITMASK);
...
extern int __pthread_sigmask (int __how, const sigset_t *__newmask,
			      sigset_t *__oldmask);
...
hidden_proto (__pthread_sigmask)
#endif
